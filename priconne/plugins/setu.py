...
@on_command('seina', aliases=('来点星奏',))
async def seina(session: CommandSession):
    # seg_setu = MessageSegment.image('http://andong.ml/static/img/%E6%98%9F%E5%A5%8F.png')
    await session.send('http://andong.ml/static/img/%E6%98%9F%E5%A5%8F.png')
    group_id = session.ctx['group_id']
    user_id = session.ctx['user_id']
    await session.bot.set_group_ban(group_id=group_id , user_id=user_id, duration=1*60)


@on_command('我有个朋友说他好了', only_to_me=False)
